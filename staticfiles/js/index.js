...
promptForm.addEventListener("submit", (event) => {
    ...
    if (promptWindow.promptList.length >= MAX_PROMPTS){
        ...
        signUpPrompt.classList.add("scale-100")
        signUpPrompt.classList.remove("scale-0")
...
function closeNavDropdown(event){
    ...
    navDropdown.classList.remove("opacity-100", "scale-100", 
        "max-lg:min-h-[450px]", "min-w-[320px]", "max-lg:!h-fit",)
...
function openVideo(){
    videoBg.classList.remove("scale-0", "opacity-0")
    videoBg.classList.add("scale-100", "opacity-100")
...
function closeVideo(){
    ...
    setTimeout(() => {
        videoBg.classList.remove("scale-100", "opacity-100")
        videoBg.classList.add("scale-0", "opacity-0")
    }, 400)
